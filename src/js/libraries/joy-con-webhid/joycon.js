...
class JoyCon extends EventTarget {
     ...
     * @private
     */
    _onInputReport({ data, reportId, device }) {
        if (!data) {
            return;
        }
        // Convert DataView to Uint8Array
        const fullData = concatTypedArrays(new Uint8Array([reportId]), new Uint8Array(data.buffer));
        ...
        switch (reportId) {
            ...
            case 0x30: {
                packet = {
                    ...
                    buttonStatus: PacketParser.parseCompleteButtonStatus(fullData, hexData),
                    analogStickLeft: PacketParser.parseAnalogStickLeft(fullData, hexData),
                    analogStickRight: PacketParser.parseAnalogStickRight(fullData, hexData),
                ...
                if (reportId === 0x21) {
                    packet = {
                        ...
                        subcommandReplyData: PacketParser.parseSubcommandReplyData(fullData, hexData),
                        deviceInfo: PacketParser.parseDeviceInfo(fullData),
                    };
                }
                if (reportId === 0x30) {
                    const accelerometers = PacketParser.parseAccelerometers(fullData, hexData);
                    const gyroscopes = PacketParser.parseGyroscopes(fullData, hexData);
                    const rps = PacketParser.calculateActualGyroscope(gyroscopes.map((g) => g.map((v) => v.rps ?? 0)));
                    const dps = PacketParser.calculateActualGyroscope(gyroscopes.map((g) => g.map((v) => v.dps ?? 0)));
                    const acc = PacketParser.calculateActualAccelerometer(accelerometers.map((a) => [
                        a.x.acc ?? 0,
                        a.y.acc ?? 0,
                        a.z.acc ?? 0,
                    ]));
                    ...
                    packet = {
                        ...
                        actualOrientation: PacketParser.toEulerAngles(this.lastValues, rps, acc, device.productId),
                        actualOrientationQuaternion: PacketParser.toEulerAnglesQuaternion(this.quaternion),
...
class JoyConLeft extends JoyCon {
    ...
    _receiveInputEvent(packet) {
        const buttonStatus = packet.buttonStatus;
